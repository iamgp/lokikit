...
@patch('builtins.print')
def test_find_grafana_binary_by_find_command(mock_print, mock_run, mock_access, mock_isfile, mock_glob, nested_temp_dir):
...
def test_get_grafana_binary_path(mock_find_grafana_binary, temp_dir, binary_info):
    ...
    assert path == expected_path
    mock_find_grafana_binary.assert_called_once_with(
        temp_dir,
        "grafana-server",
        "9.0.0"
    )
